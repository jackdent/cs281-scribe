...
\smallskip
\smallskip
\textit{Exercise}. Let $p \sim \operatorname{Bern}(r)$ and $q \sim \operatorname{Bern}(s)$, such that $P(p=0) = 1-r$, $P(p=1) = r$, $P(q=0) = 1-s$, and $P(q=1) = s$. Compute $KL(p||q)$ and $KL(q||p)$ for $r=0.5$ and $s=.25$. For what values of $r$ and $s$ does $KL(p||q)=KL(q||p)$? When does $KL(p||q)=0$? Provide an intuitive explanation for each of your answers.

\smallskip
\smallskip
\textit{Exercise}. Let $X$ and $Y$ be two random variables with joint distribution $A$ given by
...
What distribution $B$ over independent $X$ and $Y$ minimizes the the KL-divergence $KL(A||B)$? What distribution minimizes $KL(B||A)$?
